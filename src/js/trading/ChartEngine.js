...
class ChartEngine {
        ...
        this.container.innerHTML = `
            ...
            ">
                ...
                ">
                    ...
                    <button class="chart-ctrl-btn" id="btn-zoom-fit" title="Fit All"  style="${this._ctrlStyle()}">⊡</button>
                </div>
                <!-- EMA Legend + Timeframe Badge -->
                <div style="position:absolute;top:4px;left:8px;display:flex;gap:10px;font-size:10px;font-weight:600;z-index:15;pointer-events:none;align-items:center;">
                    <span style="background:rgba(99,102,241,0.25);border:1px solid rgba(99,102,241,0.5);color:#818cf8;padding:2px 7px;border-radius:4px;font-size:10px;font-weight:800;letter-spacing:0.05em;">5M</span>
                    <span id="ema-legend-9"  style="color:#f59e0b;">● EMA9</span>
                    <span id="ema-legend-21" style="color:#a78bfa;">● EMA21</span>
                    <span id="chart-candle-count" style="color:rgba(255,255,255,0.3);font-size:9px;"></span>
                </div>
            </div>
        `;
    ...
    _bindEvents() {
        ...
        // Control buttons
        document.getElementById('btn-zoom-in') ?.addEventListener('click', () => this._zoom(0.6));
        document.getElementById('btn-zoom-out')?.addEventListener('click', () => this._zoom(1.6));
    ...
    _zoom(factor, clientX = null) {
        ...
        let anchorFrac = 0.85; // anchor near right (latest candle) by default
        if (clientX !== null) {
            const rect = this.container.getBoundingClientRect();
    ...
    render() {
        ...
        // Update candle count badge
        const cntEl = document.getElementById('chart-candle-count');
        if (cntEl) cntEl.textContent = `${this.viewStart + 1}–${this.viewEnd} / ${this.data.length} candles`;
    }
    ...
    _drawXAxis(slice, barSlot, priceH) {
        ...
        // Adaptive step: show ~7 labels max, never less than 1
        const step = Math.max(1, Math.ceil(slice.length / 7));
        for (let i = 0; i < slice.length; i += step) {
            const x   = this.scaleX(i);
            const lbl = slice[i].time || `T${this.viewStart + i}`;

            // Detect new-day boundary (HH:MM label wrapping past midnight)
            const isNewDay = i > 0 && lbl.startsWith('00:');
            if (isNewDay) {
                // Draw a subtle vertical day-separator line
                this.svg.appendChild(this._makeSVG('line', {
                    x1: x, y1: top, x2: x, y2: top + priceH,
                    stroke: 'rgba(255,255,255,0.06)', 'stroke-width': '1', 'stroke-dasharray': '3 4',
                }));
            }

            const txt = this._makeSVG('text', {
                x, y,
                fill: isNewDay ? 'rgba(255,255,255,0.5)' : this.config.colors.text,
                'font-size': '9px', 'text-anchor': 'middle', 'font-family': 'inherit',
                'font-weight': isNewDay ? '700' : '400',
            });
            txt.textContent = lbl;
            this.svg.appendChild(txt);
        }
